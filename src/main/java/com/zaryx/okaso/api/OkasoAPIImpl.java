...
public class OkasoAPIImpl implements OkasoAPI {
    ...
    private OkasoSerializer serializer;
    ...
    public OkasoAPIImpl(Logger logger, Object owner, OkasoConfig config) {
        this.logger = logger;
        ...
        this.stats = new OkasoStats();

        logger.info("OkasoAPI created for: " + ownerName);
    ...
    public synchronized boolean initialize() {
        if (state != State.CREATED && state != State.DISABLED) {
            logger.warning("Okaso already initialized or in invalid state (state: " + state + ")");
            return state != State.ERROR;
        }

        try {
            initStartTime = System.currentTimeMillis();
            logger.info("Initializing Okaso for " + ownerName + "...");
            ...
            if (serializer == null) {
                serializer = new OkasoSerializer(new GsonBuilder().serializeNulls().create());
            }
            if (hasher == null) {
            ...
            logger.info("✓ Okaso initialized in " + initTime + "ms");
            return true;
    ...
    public synchronized boolean start() {
        if (state != State.INITIALIZED) {
            logger.warning("Okaso must be in INITIALIZED state (current: " + state + ")");
            return false;
        }
        ...
        try {
            ...
            state = State.RUNNING;
            logger.info("✓ Okaso started successfully");
            return true;
    ...
    public synchronized void shutdown() {
        ...
        try {
            ...
            logger.info("✓ Okaso stopped cleanly");
    ...
    @Override
    public OkasoConfig getConfig() {
        return config;
    }

    @Override
    public OkasoStats getStats() {
        updateStats();
        return stats;
    }
    ...
    public boolean reloadConfig() {
        try {
            logger.info("Reloading okaso configuration...");
            if (cacheManager != null) {
                cacheManager.clear();
            }
    ...
    public synchronized boolean reset() {
        try {
            ...
            messages = null;
            executorService = null;
            stats = new OkasoStats();
            state = State.CREATED;
            logger.info("Okaso reset complete");
            return true;
        } catch (Exception e) {
